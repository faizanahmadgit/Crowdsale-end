...
abstract contract MintedCrowdsale is Crowdsale, MintableToken {
  ...
  function _deliverTokens(
    ...
    override 
  {
    require(MintableToken.mint(_beneficiary, _tokenAmount));
  }
}
    ...
    function finalization() internal override {
      if(goalReached()){
       ...
       TokenTimelock foundersTimelock    = new TokenTimelock(token, foundersFund, releaseTime);
        ...
        MintableToken.mint(address(foundersTimelock) , _finalTotalSupply / (foundersPercentage));
        MintableToken.mint(address(foundationTimelock), _finalTotalSupply / foundersPercentage);
        MintableToken.mint(address(partnersTimelock), _finalTotalSupply / (foundersPercentage));
